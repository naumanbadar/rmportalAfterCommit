...
import org.ow2.proactive_grid_cloud_portal.client.presenter.MonitorPresenterImpl;
...
import org.ow2.proactive_grid_cloud_portal.client.view.MonitorView;
...
public class AppController implements Presenter, ValueChangeHandler<String> {
	 ...
	 *            the RPC service
	 * @param eventBus
	 *            the manager used in handling the events
	 */
	public AppController(ResourceManagerServiceAsync rpcService,
	...
	public void onValueChange(ValueChangeEvent<String> event) {
		...
		if (token != null) {
			...
			if (token.equals("login")) {
				...
				// String message =
				// LogoutCause.getCause(Integer.parseInt(itemParser.getParameterValue("code"))).getMessage();
				// presenter = new LoginPresenterImpl(rpcService, eventBus, new
				// LoginView(message));
			} else if (token.equals("rmanager")) {
				String sessionId = Cookies.getCookie(itemParser.getParameterValue("username") + "-sid");
				if (sessionId == null) {
					// History.newItem("logout?code="+
					// LogoutCause.SESSION_EXPIRED);
					return;
				}
